...
function getAgeWhenDiagnosed($, dobId, registrationId){
    ...
    var estomatoDate = getField($, dobId);
    var dateOfBirth = utils.convertEstomatoDateToJSDate(estomatoDate);

    var regs = getField($, registrationId);
...
function getCity($, cityId){
    ...
    var value = getField($, cityId);
    var index = value.indexOf('Estado:');
...
function getState($, cityId){
    ...
    return partial.replace('Estado:', '').trim();
}
